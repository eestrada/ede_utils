...
public:
    ...
    explicit list<T>(unsigned int n, const T& value = T()):
    std::list<T>(n, value){};

    template <class InputIterator>
        list(InputIterator first, InputIterator last):
        std::list<T>(first, last){};

    list<T>(const std::list<T> &other): std::list<T>(other){};
    
    /**
     * Index operator. Inefficient, but more convenient than iterators.
    ...
    /**
     * At member function. Inefficient, but more convenient than iterators.
     *
     * @param index Index within list to access.
     * @return A reference to the value at the index listed. As a reference it can both be read from and written to.
     */
